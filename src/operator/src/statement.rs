                ...
                match var_name.as_str() {
                    ...
                    _ => {
                        return NotSupportedSnafu {
                            feat: format!("Unsupport set variable {}", var_name),
                        }
                        .fail()
                    }
                }
...
fn set_timezone(exprs: Vec<Expr>, ctx: QueryContextRef) -> Result<()> {
    ...
    match tz_expr {
        ...
        expr => NotSupportedSnafu {
            feat: format!("Unsupport timezone expr {} in set variable statement", expr),
        }
        .fail(),
    }
}
...
fn to_copy_database_request(
    ...
    let start_timestamp = extract_timestamp(&arg.with, COPY_DATABASE_TIME_START_KEY, tz.clone())?;
    let end_timestamp = extract_timestamp(&arg.with, COPY_DATABASE_TIME_END_KEY, tz)?;
...
fn extract_timestamp(map: &OptionMap, key: &str, tz: Timezone) -> Result<Option<Timestamp>> {
    map.get(key)
        .map(|v| {
            Timestamp::from_str(v, Some(tz))
