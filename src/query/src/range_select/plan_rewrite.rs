...
/// 3. leave empty (as Default Option):
///     - user specifies the time zone in query context: align to user specify tz
///     - else : align to unix epoch 0
fn parse_align_to(args: &[Expr], i: usize, timezone: &Timezone) -> DFResult<i64> {
    let mut s = parse_str_expr(args, i)?.to_string();
    match s.to_uppercase().as_str() {
        "NOW" => return Ok(Timestamp::current_millis().value()),
        "" => s = "1970-01-01 00:00:00".to_string(),
        _ => (),
    }
    Timestamp::from_str(s.as_str(),Some(timezone.clone()))
        .map_err(|e| {
    ...
    fn mutate(&mut self, node: Expr) -> DFResult<Expr> {
        if let Expr::ScalarUDF(func) = &node {
            if func.fun.name == "range_fn" {
                ...
                let align_to = parse_align_to(&func.args, byc + 5, self.tz)?;
...
pub struct RangePlanRewriter {
    ...
    // Requires user-specified time zone in query context to determine default range align behavior
    timezone: Timezone,
}

impl RangePlanRewriter {
    pub fn new(table_provider: DfTableSourceProvider, timezone: Timezone) -> Self {
        Self {
            table_provider,
            timezone,
        }
...
mod test {
    ...
    fn test_parse_align_to() {
        let utc_tz = Timezone::from_tz_string("UTC").unwrap();
        let tz1 = Timezone::from_tz_string("+08:00").unwrap();
        let tz2 = Timezone::from_tz_string("Asia/Shanghai").unwrap();
        let chinese_time = -8 * 60 * 60 * 1000;
        // test NOW
        let args = vec![Expr::Literal(ScalarValue::Utf8(Some("NOW".into())))];
        let epsinon =
            parse_align_to(&args, 0, &utc_tz).unwrap() - Timestamp::current_millis().value();
        assert!(epsinon.abs() < 100);
        // test Timestamp
        let args = vec![Expr::Literal(ScalarValue::Utf8(Some(
            "1970-01-01T00:00:00+08:00".into(),
        )))];
        assert_eq!(parse_align_to(&args, 0, &utc_tz).unwrap(), chinese_time);
        let args = vec![Expr::Literal(ScalarValue::Utf8(Some(
            "1970-01-01T00:00:00".into(),
        )))];
        assert_eq!(parse_align_to(&args, 0, &tz1).unwrap(), chinese_time);
        // test default with tz
        let args = vec![Expr::Literal(ScalarValue::Utf8(Some("".into())))];
        assert_eq!(parse_align_to(&args, 0, &utc_tz).unwrap(), 0);
        assert_eq!(parse_align_to(&args, 0, &tz1).unwrap(), chinese_time);
        assert_eq!(parse_align_to(&args, 0, &tz2).unwrap(), chinese_time);
    }
}
